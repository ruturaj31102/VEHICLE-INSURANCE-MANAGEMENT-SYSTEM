...
public class updateinsert extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					updateinsert frame = new updateinsert();
	...
	public updateinsert() {
		...
		lblInsertid.setBounds(314, 15, 142, 14);
		contentPane.add(lblInsertid);
		
		id = new JTextField();
		id.setBounds(494, 14, 86, 20);
		contentPane.add(id);
		id.setColumns(10);
		...
		btnUpdate.setFont(new Font("Monospaced", Font.BOLD, 17));
		btnUpdate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				try
				{
					...
					Connection con=DriverManager.getConnection("jdbc:mysql://localhost:8111/vims","root","");
					String query = " update vehicle set name=?,mobile_no=?,vtype=?,vehicle_no=?,rc_no=?,purchase_date=?,location=?,pcommence=?,pexpiry=? where policy_id=?";
					...
					ps.setString(9,expire);
					ps.setString (10,policyid);
					ps.execute();
				...
				{
					System.out.println("Data not updated"+e2.getMessage());
				}
				
			}
		});
		btnUpdate.setBounds(412, 400, 99, 23);
		...
		btnOk.setFont(new Font("Monospaced", Font.BOLD, 17));
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				try
				{
					...
					while(rs.next())
					{
						...
						String Policy_start_Date=rs.getString(9);
						String Policy_Expiry_Date=rs.getString(10);
					
						tn.setText(name);
						tm.setText(Mobile_No);
						ta.setText(Address);
						vt.setSelectedItem(VehicleType);
						tvn.setText(Vehicle_No);
						trc.setText(Rc_No);
						vpd.setText(Purchase_Date);
						pcd.setText(Policy_start_Date);
						ped.setText(Policy_Expiry_Date);
		...
		btnOk.setBounds(491, 45, 89, 23);
		contentPane.add(btnOk);
		
		JLabel label = new JLabel("------------------------------------------------------------------");
		label.setBounds(310, 72, 280, 14);
